-- test5@gmail.com 계정의 기존 할일 삭제 후 더미 데이터 추가

-- 1단계: 기존 할일들 삭제
DELETE FROM todos 
WHERE user_id = (SELECT id FROM auth.users WHERE email = 'test5@gmail.com');
...
(
    ...
    100,
    true,
    (SELECT id FROM auth.users WHERE email = 'test5@gmail.com'),
    NOW(),
    NOW()
),
(
